...
namespace BIMVision
{
    ...
    public struct SDK
    {
        ...
        public const int API_VERSION_MINOR = 52;
    }
    ...
    public abstract class Plugin
    {
        ...
        {
            plugin.major = GetApiVersionMajor();
            plugin.minor = GetApiVersionMinor();
        ...
        public virtual int GetApiVersionMinor()
        {
            return SDK.API_VERSION_MINOR;
        }

        public virtual int GetApiVersionMajor()
        {
            return SDK.API_VERSION_MAJOR;
        }
    ...
    public class PluginDll
    {
        ...
        [DllExport("get_api_verion_minor")]
        private static int get_api_verion_minor()
        {
            return CurrentPlugin.GetApiVersionMinor();
        }

        [DllExport("get_api_verion_major")]
        private static int get_api_verion_major()
        {
            return CurrentPlugin.GetApiVersionMajor();
        }
    ...
    {
        public ulong id;
        public override bool Equals(object obj) /* składnia dla v6 */
        {
            if (obj is OBJECT_ID)
            {
                OBJECT_ID iD = (OBJECT_ID)obj;
                return id == (ulong)iD.id;
            }
            return false;
        }
        /*	public override bool Equals(object obj)
            {
              var iD = obj as OBJECT_ID;
    ...
    public struct SOLID_ID : IEquatable<SOLID_ID>
    {
        ...
        public override bool Equals(object obj) /* składnia dla v6 */
        {
            if (obj is SOLID_ID)
            {
                SOLID_ID iD = (SOLID_ID)obj;
                return id == (Int64)iD.id;
            }
            return false;
        }
    ...
    public class Api
    {
        ...
        public static fun_get_all_branch_childrens get_all_branch_childrens;
        ...
        public static fun_get_all_groups1 get_all_groups1;
        ...
        public static fun_set_new_timer set_new_timer;
        public static fun_reset_timer reset_timer;
        ...
        public static void loadPluginFunctions()
        {
            ...
            get_all_branch_childrens = (fun_get_all_branch_childrens)LoadFunction<fun_get_all_branch_childrens>("get_all_branch_childrens");
            ...
            get_all_groups1 = (fun_get_all_groups1)LoadFunction<fun_get_all_groups1>("get_all_groups1");
            ...
            set_new_timer = (fun_set_new_timer)LoadFunction<fun_set_new_timer>("set_new_timer");
            reset_timer = (fun_reset_timer)LoadFunction<fun_reset_timer>("reset_timer");
        }

        public delegate void callback_fun();
        public delegate void timer_callback_fun(IntPtr param);
        ...
        public delegate uint fun_get_all_branch_childrens(PLUGIN_ID pid, OBJECT_ID id, [Out] OBJECT_ID[] buf, uint count);
        ...
        public delegate uint fun_get_all_groups1(PLUGIN_ID pid, [Out] Group1_[] buf, uint count);
        ...
        public delegate TIMER_REF fun_set_new_timer(uint milsec, IntPtr fun, IntPtr param);
        public delegate void fun_reset_timer(TIMER_REF timer_ref, bool enable, uint milsec);
    ...
    public class ApiWrapper
    {
        ...
        public OBJECT_ID[] GetAllBranchChildrens(OBJECT_ID id)
        {
            var count = Api.get_all_branch_childrens(pid, id, null, 0);
            if (count > 0)
            {
                var res = new OBJECT_ID[count];
                Api.get_all_branch_childrens(pid, id, res, count);
                return res;
            }
            else
                return null;
        ...
        public struct Group1
        {
            ...
            public string object_type;
            public string global_id;
            public string entity_org_name;
        }

        public Group1[] GetAllGroups1()
        {
            var count = Api.get_all_groups1(pid, null, 0);
            if (count > 0)
            {
                Group1_[] groups = new Group1_[count];

                count = Math.Min(count, Api.get_all_groups1(pid, groups, (uint)groups.Length));
                var res = new Group1[count];
                for (var i = 0; i < count; i++)
                {
                    res[i].group_ref = groups[i].group_ref;
                    res[i].name = Marshal.PtrToStringUni(groups[i].name);
                    ...
                    res[i].object_type = Marshal.PtrToStringUni(groups[i].object_type);
                    res[i].global_id = Marshal.PtrToStringAnsi(groups[i].global_id);
                    res[i].entity_org_name = Marshal.PtrToStringAnsi(groups[i].entity_org_name);
                }
                return res;
            }
            else
        ...
        private static Api.timer_callback_fun onTimerCallbackDelegate;
        public TIMER_REF SetNewTimer(uint milsec, Api.timer_callback_fun fun, IntPtr param)
        {
            onTimerCallbackDelegate = fun;
            return Api.set_new_timer(milsec, Marshal.GetFunctionPointerForDelegate(fun), param);
        }

        public void ResetTimer(TIMER_REF timer_ref, bool enable, uint milsec)
        {
            Api.reset_timer(timer_ref, enable, milsec);
        }
